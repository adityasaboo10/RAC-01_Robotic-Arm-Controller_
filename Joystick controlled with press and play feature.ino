...
// Press-and-play setup
#define MAX_POSITIONS 6
#define LONG_PRESS_DURATION 800

struct Position {
  int base, elbow, arm, claw;
};

Position positions[MAX_POSITIONS];
int pp_positionCount = 0;
bool pp_lastButtonState = LOW;
unsigned long pp_pressStartTime = 0;
bool pp_playbackMode = false;

void checkRecordingButton() {
  bool pp_currentState = digitalRead(pp_recordButtonPin);

  // Button just pressed
  if (pp_currentState == HIGH && pp_lastButtonState == LOW) {
    pp_pressStartTime = millis();
  }

  // Button just released
  if (pp_currentState == LOW && pp_lastButtonState == HIGH) {
    unsigned long pressDuration = millis() - pp_pressStartTime;
    ...
    if (pressDuration >= LONG_PRESS_DURATION) {
      ...
      if (pp_positionCount < MAX_POSITIONS) {
        ...
        Serial.println(pp_positionCount);
      }
    }
  }

  pp_lastButtonState = pp_currentState;
}

void playback() {
  for (int i = 0; i < pp_positionCount; i++) {
    reset(servobase, servobasepos, positions[i].base, speed);
    reset(servoelbow, servoelbowpos, positions[i].elbow, speed);
    reset(servoarm, servoarmpos, positions[i].arm, speed);
...
void setup() {
  ...
  pinMode(led3, OUTPUT);
  pinMode(led4, OUTPUT);
  pinMode(pp_recordButtonPin, INPUT);
