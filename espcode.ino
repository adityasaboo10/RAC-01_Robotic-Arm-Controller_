...
void readJoystickTask(void *parameter) {
  for (;;) {
    read();  // Your joystick reading function
    vTaskDelay(50 / portTICK_PERIOD_MS);  // delay to avoid overwhelming core
  }
}
...
void setup() {
  ...
  // Attach servos
  servobase.attach(servobasepin, 500, 2400);      //(servoPin, minUs, maxUs)
  servoelbow.attach(servoelbowpin, 500, 2400);
  servoarm.attach(servoarmpin, 500, 2400);
  servoclaw.attach(servoclawpin, 500, 2400);
...
/* FOR Reducing core load */
  xTaskCreatePinnedToCore(
  readJoystickTask,    // Function
  "ReadJoystick",      // Name
  10000,               // Stack size
  NULL,                // Parameters
  1,                   // Priority
